...
void getMPU6050() {
  ...
  lorasend[0] = ypr[2] * 180.0 / PI;       // X 軸角度（Roll）
  lorasend[1] = ypr[1] * 180.0 / PI;       // Y 軸角度（Pitch）
  lorasend[2] = gyro.z / 131.0;            // Z 軸角速度（°/s）
  lorasend[3] = aaReal.x / 16384.0 * 9.8;  // X 加速度（m/s²）
  lorasend[4] = aaReal.y / 16384.0 * 9.8;  // Y 加速度（m/s²）
  lorasend[5] = aaReal.z / 16384.0 * 9.8;  // Z 加速度（m/s²）
}
