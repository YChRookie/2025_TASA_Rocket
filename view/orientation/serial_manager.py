...
class SerialManager:
    ...
    # def __parse_raw_data(self, data: bytes, mode: str = "struct") -> tuple:
    #     return struct.unpack(f"{self.__pack_length}f", data)

    def __parse_raw_data(self, data: bytes, mode: str = "struct") -> tuple:
        try:
            return struct.unpack(f"<{self.__pack_length}f", data)  # 添加 < 表示小端序
        except struct.error as e:
            # 這是你需要詳細查看的錯誤訊息
            self.__log_error(
                f"Struct unpack error: {e}. Raw data length: {len(data)} bytes. Expected pack size: {self.__pack_size} bytes. Hex data: {data.hex()}"
            )
            raise  # 重新拋出，讓外層捕獲並停止，或進行其他處理
        except Exception as e:
            self.__log_error(
                f"Unexpected error in parse_raw_data: {e}. Raw data length: {len(data)} bytes. Hex data: {data.hex()}"
            )
            raise
    ...
    def __read_serial_data(self):
        ...
        try:
            while self.serial.in_waiting >= self.__pack_size:
                raw_data = self.serial.read(self.__pack_size)
                print(f"Received raw_data length: {len(raw_data)} bytes")
                print(f"Received raw_data (hex): {raw_data.hex()}")
                print(f"Expected pack size: {self.__pack_size} bytes")
            ...
            self.__log_error(e)
            print(f"Error occurred while processing raw_data (hex): {raw_data.hex()}")
    ...
    def __write_all_to_db(
        ...
        self.__db_mgr.insert_to_table(
            "orientation",
            "x_angle, y_angle, z_angular_velocity",
            (x_angle, y_angle, z_angular_velocity),
        )
        ...
        self.__db_mgr.insert_to_table(
            "time_info", "boot_timestamp, elapsed_time", (boot_ts, elapsed)
        )
